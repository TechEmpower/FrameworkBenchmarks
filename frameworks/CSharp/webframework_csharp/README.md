...
3. Create `webframework_csharp.dockerfile`
...
You can create multiple implementations and they will all conform to `[name in benchmark_config.json].dockerfile`. For example, the `default` implementation in `benchmark_config.json` will be `webframework_csharp.dockerfile`, but if you wanted to make another implementation that did only the database tests for MySQL, you could make `webframework_csharp-mysql.dockerfile` and have an entry in your `benchmark_config.json` for `webframework_csharp-mysql`.

4. Test your application

        $ tfb --mode verify --test webframework_csharp
...
Edit `.github/workflows/build.yml` to ensure that Github Actions will automatically run our verification tests against your new test. This file is kept in alphabetical order, so find where `TESTDIR=CSharp/webframework_csharp` should be inserted under `env > matrix` and put it there.
...
# WebFramework_CSharp Benchmarking Test
