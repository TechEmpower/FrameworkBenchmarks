...
RUN apt update
RUN apt install -y gcc g++ cmake git unzip zip wget ninja-build uuid-dev dotnet-sdk-10.0
...
RUN cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/opt/WebFrameworkLibrary -DCMAKE_CXX_FLAGS="-march=native" -DCMAKE_C_FLAGS="-march=native" \
	-DBUILD_CSHARP_API=ON -DBUILD_PYTHON_API=OFF -DBUILD_CC_API=OFF -DWITH_DOTNET_EXECUTORS=ON -DWITH_PYTHON_EXECUTORS=OFF -G "Ninja" ..
RUN cmake --build . -j
RUN cmake --install .
...
EXPOSE 8080
CMD LD_LIBRARY_PATH=$(pwd) dotnet Benchmark.dll
