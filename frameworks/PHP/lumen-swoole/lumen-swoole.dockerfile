...
ADD ./ /lumen-swoole
WORKDIR /lumen-swoole
COPY php.ini /usr/local/etc/php/

RUN mkdir -p /lumen-swoole/storage/framework/sessions
RUN mkdir -p /lumen-swoole/storage/framework/views
RUN mkdir -p /lumen-swoole/storage/framework/cache
RUN mkdir -p /lumen-swoole/storage/logs

RUN chmod -R 777 /lumen-swoole
