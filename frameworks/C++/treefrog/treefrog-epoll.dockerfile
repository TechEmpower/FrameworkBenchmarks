...
RUN apt-get update -yqq > /dev/null && apt-get upgrade -yq > /dev/null && \
    ...
    libmongoc-dev libbson-dev gcc g++ clang make cmake pkg-config > /dev/null
...
RUN tar xf v${TFVER}.tar.gz > /dev/null
