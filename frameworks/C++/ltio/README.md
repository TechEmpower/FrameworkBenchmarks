...
You can create multiple implementations and they will all conform to `[name in benchmark_config.json].dockerfile`. For example, the `default` implementation in `benchmark_config.json` will be `ltio.dockerfile`, but if you wanted to make another implementation that did only the database tests for MySQL, you could make `ltio-mysql.dockerfile` and have an entry in your `benchmark_config.json` for `ltio-mysql`.

4. Test your application

        $ tfb --mode verify --test ltio
...
Edit `.github/workflows/build.yml` to ensure that Github Actions will automatically run our verification tests against your new test. This file is kept in alphabetical order, so find where `TESTDIR=c++/ltio` should be inserted under `env > matrix` and put it there.
