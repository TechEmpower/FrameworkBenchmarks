...
RUN  apt-get update -yqq > /dev/null && \
     ...
     zlib1g-dev  > /dev/null && \
     add-apt-repository ppa:ubuntu-toolchain-r/test -y > /dev/null && \
...
RUN git checkout 96919df488e0ebaa0ed304bbd76bba33508df3cc > /dev/null
...
RUN make --quiet && make install --quiet
