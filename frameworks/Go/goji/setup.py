...
def start(args, logfile, errfile):
  setup_util.replace_text("goji/src/app/app.go", "tcp\(.*:3306\)", "tcp(" + args.database_host + ":3306)")
  if os.name == 'nt':
    ...
    subprocess.call("set GOPATH=C:\\FrameworkBenchmarks\\go&& go get ./...", shell=True, cwd="goji", stderr=errfile, stdout=logfile)
    subprocess.Popen("setup.bat", shell=True, cwd="goji", stderr=errfile, stdout=logfile)
    return 0
  ...
  subprocess.Popen("go run src/app/app.go src/app/models.go".rsplit(" "), cwd="goji", stderr=errfile, stdout=logfile)
  return 0
