...
3. Create `exposed vert.x sql client.dockerfile`
...
You can create multiple implementations and they will all conform to `[name in benchmark_config.json].dockerfile`. For example, the `default` implementation in `benchmark_config.json` will be `exposed vert.x sql client.dockerfile`, but if you wanted to make another implementation that did only the database tests for MySQL, you could make `exposed vert.x sql client-mysql.dockerfile` and have an entry in your `benchmark_config.json` for `exposed vert.x sql client-mysql`.

4. Test your application

        $ tfb --mode verify --test exposed vert.x sql client
...
Edit `.github/workflows/build.yml` to ensure that Github Actions will automatically run our verification tests against your new test. This file is kept in alphabetical order, so find where `TESTDIR=Kotlin/exposed vert.x sql client` should be inserted under `env > matrix` and put it there.
