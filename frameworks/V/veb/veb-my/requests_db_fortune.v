...
pub fn (app &App) fortunes(mut ctx Context) veb.Result {
	fortunes_map := app.db.exec('SELECT id, message FROM ${db_table_fortune}') or {
		return ctx.server_error('Failed to fetch ${db_table_fortune}: ${err}')
	}

	mut fortunes := []Fortune{}
	for fortune in fortunes_map {
		id := fortune.vals[0]
		message := fortune.vals[1]
		fortunes << convert_fortune(id, message)
	}
...
fn db_init_fortune(app App, mut ctx Context) (int, string) {
	app.db.exec('DROP TABLE IF EXISTS ${db_table_fortune}') or {
		return 1, 'Failed to drop existing Fortune table'
	}
	app.db.exec('CREATE TABLE ${db_table_fortune} (
		id SERIAL PRIMARY KEY,
		message ${db_connector_varchar} NOT NULL
	)') or {
		return 1, 'Failed to create ${db_table_fortune} table'
	}
	...
	app.db.exec('INSERT INTO ${db_table_fortune} (message) VALUES (\'${message}\')') or {
		return 1, 'Failed to insert ${db_table_fortune} messages: ${err}'
	}
