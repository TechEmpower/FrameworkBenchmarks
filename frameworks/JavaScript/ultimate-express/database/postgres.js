...
export const find = async (id) => {
  const res = await sql`SELECT id, randomNumber FROM world WHERE id = ${id}`
  return res[0]
}

export const getAllWorlds = () => sql`SELECT id, randomNumber FROM world`

export const bulkUpdate = (worlds) => sql`UPDATE world SET randomNumber = (update_data.randomNumber)::int
