...
public class RawOfficeFloorMain {
     ...
     * {@link Logger}.
     */
    private static final Logger logger = Logger.getLogger(RawOfficeFloorMain.class.getName());
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (CpuCore cpuCore : cpuCores) {
            cpuCount += cpuCore.getCpus().length;
        }
    ...
    private static class RawHttpServicerFactory extends AbstractHttpServicerFactory {
        ...
        private static final HttpHeaderName NAME_SERVER = new HttpHeaderName("Server");

        private static final HttpHeaderValue VALUE_SERVER = new HttpHeaderValue("OF");

        private static final HttpHeaderName NAME_DATE = new HttpHeaderName("Date");
         ...
         */
        private static final ManagedObjectContext managedObjectContext = new ManagedObjectContext() {
        ...
        private void db(HttpResponse response, ProcessAwareServerHttpConnectionManagedObject<ByteBuffer> connection) {
            ...
            }, error -> this.sendError(connection, error), () -> conn.processed(1));
        }
        ...
        private void queries(String requestUri, HttpResponse response,
            ...
            }, error -> this.sendError(connection, error), () -> conn.processed(queryCount));
        }
        ...
        private void fortunes(HttpResponse response,
            ...
            Flux.from(conn.connection.createStatement("SELECT ID, MESSAGE FROM FORTUNE").execute())
                ...
                try {
                    ...
                    fortunes.sort(Comparator.comparing(a -> a.message));
        ...
        private void update(String requestUri, HttpResponse response,
            ...
            Flux.range(1, queryCount)
                ...
                worlds.sort(Comparator.comparingInt(a -> a.id));
            ...
            }, error -> this.sendError(connection, error), () -> conn.processed(executeQueryCount));
        }
        ...
        private static int getQueryCount(String queries) {
            try {
                ...
                return (count < 1) ? 1 : Math.min(count, 500);
    ...
    private static class RateLimit {
        ...
        private RateLimit(RequestHandler<HttpRequestParser> requestHandler, Connection[] connections) {
            ...
            Executor socketExecutor = (runnable) -> requestHandler.execute(runnable::run);
            Scheduler writeScheduler = Schedulers.fromExecutor(socketExecutor);
        ...
        private RateLimitedConnection getAvailableConnection(int queryCount) {
            ...
            for (RateLimitedConnection connection : this.rateLimitedConnections) {
