...
def start(args, logfile, errfile):
  ...
  try:
    subprocess.check_call("rvm 2.1.2 do bundle install --gemfile=Gemfile-ruby", shell=True, cwd="rails", stderr=errfile, stdout=logfile)
    ...
    subprocess.Popen("rvm 2.1.2 do bundle exec unicorn_rails -E production -c config/unicorn.rb", shell=True, cwd="rails", stderr=errfile, stdout=logfile)
    return 0
  ...
  try:
    ...
    for line in out.splitlines():
      if 'unicorn' in line and 'master' in line:
        ...
        os.kill(pid, 15)
    # subprocess.check_call("rvm 2.1.2 do bundle exec passenger stop --pid-file=$HOME/FrameworkBenchmarks/rack/rack.pid", shell=True, cwd='rack')
