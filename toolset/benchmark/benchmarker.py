...
from utils.metadata_helper import gather_frameworks, gather_tests, gather_remaining_tests
...
class Benchmarker:
  ...
  def run_list_test_metadata(self):
    ...
    with open(os.path.join(self.results.directory, "test_metadata.json"), "w") as f:
  ...
  def __setup_server(self):
    ...
    try:
      if os.name == 'nt':
        return True
      subprocess.call(['sudo', 'sysctl', '-w', 'net.ipv4.tcp_max_syn_backlog=65535'], stdout=self.config.quiet_out, stderr=subprocess.STDOUT)
      subprocess.call(['sudo', 'sysctl', '-w', 'net.core.somaxconn=65535'], stdout=self.config.quiet_out, stderr=subprocess.STDOUT)
      subprocess.call(['sudo', 'sysctl', 'net.ipv4.tcp_tw_reuse=1'], stdout=self.config.quiet_out, stderr=subprocess.STDOUT)
      subprocess.call(['sudo', 'sysctl', 'net.ipv4.tcp_tw_recycle=1'], stdout=self.config.quiet_out, stderr=subprocess.STDOUT)
      subprocess.call(['sudo', 'sysctl', '-w', 'kernel.shmmax=134217728'], stdout=self.config.quiet_out, stderr=subprocess.STDOUT)
      subprocess.call(['sudo', 'sysctl', '-w', 'kernel.shmall=2097152'], stdout=self.config.quiet_out, stderr=subprocess.STDOUT)

      with open(os.path.join(self.results.directory, 'sysctl.txt'), 'w') as f:
  ...
  def __setup_database(self):
    ...
    '''
    p = subprocess.Popen(self.config.database_ssh_string, stdin=subprocess.PIPE, shell=True, stdout=self.config.quiet_out, stderr=subprocess.STDOUT)
    p.communicate("""
  ...
  def __setup_database_container(self, database):
    ...
    p = subprocess.Popen(self.config.database_ssh_string, stdin=subprocess.PIPE, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    ...
    if len(dbid) != 12 and not __is_hex(dbid):
      def __scp_string(files):
        ...
        for file in files:
          scpstr.append(file)
        scpstr.append("%s@%s:~/%s/" % (self.config.database_user, self.config.database_host, database))
        return scpstr

      p = subprocess.Popen(self.config.database_ssh_string, shell=True, stdin=subprocess.PIPE, stdout=self.config.quiet_out, stderr=subprocess.STDOUT)
      p.communicate("mkdir -p %s" % database)
      dbpath = os.path.join(self.config.fwroot, "toolset", "setup", "linux", "docker", "databases", database)
      dbfiles = ""
      for dbfile in os.listdir(dbpath):
        dbfiles += "%s " % os.path.join(dbpath,dbfile)
      p = subprocess.Popen(__scp_string(dbfiles.split()), stdin=subprocess.PIPE, stdout=self.config.quiet_out, stderr=subprocess.STDOUT)
      p.communicate()
      p = subprocess.Popen(self.config.database_ssh_string, shell=True, stdin=subprocess.PIPE, stdout=self.config.quiet_out, stderr=subprocess.STDOUT)
  ...
  def __setup_client(self):
    ...
    '''
    p = subprocess.Popen(self.config.client_ssh_string, stdin=subprocess.PIPE, shell=True, stdout=self.config.quiet_out, stderr=subprocess.STDOUT)
    p.communicate("""
      sudo sysctl -w net.ipv4.tcp_max_syn_backlog=65535
      sudo sysctl -w net.core.somaxconn=65535
      sudo -s ulimit -n 65535
  ...
  def __run_tests(self, tests):
    ...
    if self.config.os.lower() == 'windows':
      ...
      for test in tests:
        with open(self.config.current_benchmark, 'w') as benchmark_resume_file:
      ...
      widgets = [self.config.mode, ': ',  progressbar.Percentage(), ' ', 
                 progressbar.Bar(), ' Rough ', progressbar.ETA()]
      ...
      for test in tests:
        ...
        if __name__ == 'benchmark.benchmarker':
          ...
          self.results.load()  # Load intermediate result from child process
          if(test_process.is_alive()):
            logging.debug("Child process for {name} is still alive. Terminating.".format(name=test.name))
            self.results.write_intermediate(test.name,"__run_test timeout (="+ str(self.config.run_test_timeout_seconds) + " seconds)")
  ...
  def __run_test(self, test):
    ...
    with open(os.path.join(logDir, 'out.txt'), 'w') as out:
      ...
      if test.os.lower() != self.config.os.lower() or test.database_os.lower() != self.config.database_os.lower():
      ...
      try:
        ...
        if self.__is_port_bound(test.port):
          # We gave it our all
          self.results.write_intermediate(test.name, "port " + str(test.port) + " is not available before start")
        ...
        if test.database != "None":
          ...
          if not database_container_id:
            ...
            out.flush()
            self.results.write_intermediate(test.name,"ERROR: Problem starting")
            return sys.exit(1)
        ...
        logging.info("Sleeping %s seconds to ensure framework is ready" % self.config.sleep)
        ...
        self.results.write_intermediate(test.name,"<setup.py> raised an exception")
