...
class FrameworkTest:
    ...
    def benchmark(self, logPath):
        ...
        def benchmark_type(test_type):
            ...
            with open(os.path.join(benchmarkPath, 'benchmark.txt'),
                ...
                test = self.runTests[test_type]
                test.setup_out(out)
                output_file = self.results.get_output_file(
                    self.name, test_type)
                ...
                if not test.failed:
                    if test_type == 'plaintext':  # One special case
                        remote_script = self.__generate_pipeline_script(
                            test.get_url(), self.port, test.accept_header)
                    elif test_type == 'query' or test_type == 'update':
                        remote_script = self.__generate_query_script(
                            test.get_url(), self.port, test.accept_header,
                            self.benchmarker_config.query_levels)
                    elif test_type == 'cached_query':
                        remote_script = self.__generate_query_script(
                            test.get_url(), self.port, test.accept_header,
                            self.benchmarker_config.cached_query_levels)
                    else:
                        remote_script = self.__generate_concurrency_script(
                            test.get_url(), self.port, test.accept_header)
                    ...
                    with open(output_file, 'w') as raw_file:
                        p = subprocess.Popen(
                            self.benchmarker_config.client_ssh_string.split(
                                " "),
                ...
                self.results.report_benchmark_results(
                    framework=self, test=test_type, results=results['results'])
                out.write("Complete\n")
                out.flush()
    ...
    def parse_all(self):
        ...
        for test_type in self.runTests:
            if os.path.exists(
                    self.results.get_output_file(self.name, test_type)):
                results = self.__parse_test(test_type)
                self.results.report_benchmark_results(
                    framework=self, test=test_type, results=results['results'])
    ...
    def requires_database(self):
        ...
        '''
        return any(
            tobj.requires_db for (ttype, tobj) in self.runTests.iteritems())
    ...
    def __parse_test(self, test_type):
        ...
        try:
            ...
            if os.path.exists(
                    self.results.get_output_file(self.name, test_type)):
                with open(self.results.output_file(self.name,
                        ...
                        if not is_warmup:
                            ...
                            if "ENDTIME" in line:
                                ...
                                test_stats = self.__parse_stats(
                                    test_type, rawData["startTime"],
                                    rawData["endTime"], 1)
                                stats.append(test_stats)
            with open(
                    self.results.stats_file(self.name, test_type) + ".json",
                    "w") as stats_file:
    ...
    def __generate_concurrency_script(self,
                                      url,
                                      port,
                                      accept_header,
                                      wrk_command="wrk"):
        ...
        concurrency tests.
        '''
        headers = self.headers_template.format(
            server_host=self.benchmarker_config.server_host,
            accept=accept_header)
        return self.concurrency_template.format(
            max_concurrency=max(self.benchmarker_config.concurrency_levels),
            name=self.name,
            duration=self.benchmarker_config.duration,
            levels=" ".join(
                "{}".format(item)
                for item in self.benchmarker_config.concurrency_levels),
            server_host=self.benchmarker_config.server_host,
            port=port,
            url=url,
            headers=headers,
    ...
    def __generate_pipeline_script(self,
                                   url,
                                   port,
                                   accept_header,
                                   wrk_command="wrk"):
        ...
        return self.pipeline_template.format(
            max_concurrency=max(
                self.benchmarker_config.pipeline_concurrency_levels),
            name=self.name,
            duration=self.benchmarker_config.duration,
            levels=" ".join(
    ...
    def __begin_logging(self, test_type):
        ...
        output_file = "{file_name}".format(
            file_name=self.benchmarker_config.get_stats_file(
                self.name, test_type))
    ...
    def __parse_stats(self, test_type, start_time, end_time, interval):
        ...
        '''
        stats_dict = dict()
        stats_file = self.results.get_stats_file(self.name, test_type)
