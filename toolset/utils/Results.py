from metadata_helper import gather_remaining_tests
from output_helper import header
...
class Results:
  ...
  def __init__(self, config):
    ...
    '''
    self.config = config
    self.directory = os.path.join(self.config.fwroot, "results", self.config.timestamp)
    try:
    ...
    self.frameworks = [t.name for t in gather_remaining_tests(self.config, self)]
  ...
  def upload(self):
    ...
    if self.config.results_upload_uri != None:
      try:
        requests.post(self.config.results_upload_uri, headers={ 'Content-Type': 'application/json' }, data=json.dumps(self, indent=2))
      except (Exception):
  ...
  def get_output_file(self, test_name, test_type):
    ...
    '''
    path = os.path.join(self.directory, self.config.timestamp, test_name, test_type, "raw.txt")
    try:
  ...
  def get_stats_file(self, test_name, test_type):
    ...
    '''
    path = os.path.join(self.directory, self.config.timestamp, test_name, test_type, "stats.txt")
    try:
      os.makedirs(os.path.dirname(path))
    except OSError:
      pass
  ...
  def finish(self):
    ...
    print("Time to complete: " + str(int(time.time() - self.config.start_time)) + " seconds")
    print("Results are saved in " + os.path.join(self.directory, self.config.timestamp))
  ...
  def __count_sloc(self):
    ...
    '''
    frameworks = gather_frameworks(include=self.config.test,
                                    exclude=self.config.exclude, benchmarker_config=self.config)
