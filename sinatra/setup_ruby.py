...
def start(args, logfile, errfile):
  ...
  try:
    subprocess.check_call("rvm use 2.1.1 do bundle install --gemfile=Gemfile-ruby", shell=True, cwd="sinatra", stderr=errfile, stdout=logfile)
    ...
    subprocess.Popen("rvm use 2.1.1 do bundle exec unicorn_rails -E production -c config/unicorn.rb", shell=True, cwd="sinatra", stderr=errfile, stdout=logfile)
    return 0
