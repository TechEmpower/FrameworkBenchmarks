...
def start(args, logfile):
  ...
  try:
    subprocess.check_call("mvn clean compile war:war", shell=True, cwd="tapestry/hello", stderr=logfile, stdout=logfile)
    subprocess.check_call("rm -rf $RESIN_HOME/webapps/*", shell=True, stderr=logfile, stdout=logfile)
    subprocess.check_call("cp tapestry/hello/target/tapestry.war $RESIN_HOME/webapps/tapestry.war", shell=True, stderr=logfile, stdout=logfile)
