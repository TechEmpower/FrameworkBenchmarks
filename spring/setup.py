...
def start(args, logfile):
  ...
  try:
    subprocess.check_call("mvn clean compile war:war", shell=True, cwd="spring", stderr=logfile, stdout=logfile)
    ...
    if os.name == 'nt':
      ...
      subprocess.check_call('copy spring\\target\\spring.war "%RESIN_HOME%\\webapps\\spring.war"', shell=True, stderr=logfile, stdout=logfile)
      ...
      subprocess.check_call("cp spring/target/spring.war $RESIN_HOME/webapps/spring.war", shell=True, stderr=logfile, stdout=logfile)
