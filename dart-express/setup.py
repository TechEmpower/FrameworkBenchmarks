...
def start(args):
  setup_util.replace_text('dart-express/postgresql.yaml', 'host: .*', 'host: ' + args.database_host)
  setup_util.replace_text('dart-express/mongodb.yaml', 'host: .*', 'host: ' + args.database_host)
  try:
    #
    # install dart dependencies
    #
    subprocess.check_call('pub install', shell=True, cwd='dart-express')
    #
    # start dart servers
    #
    for port in range(9001, 9001 + args.max_threads):
      subprocess.Popen('dart server.dart -a 127.0.0.1 -p ' + str(port) + ' -d ' + str(args.max_concurrency / args.max_threads), shell=True, cwd='dart-express')
    #
    ...
    #
    with open('dart-express/nginx.conf', 'w') as f:
      f.write('\n'.join(conf))
    #
    # start nginx
    #
    subprocess.Popen('sudo /usr/sbin/nginx -c `pwd`/nginx.conf', shell=True, cwd='dart-express');
    return 0
...
def stop():
  ...
  # stop nginx
  #
  subprocess.check_call('sudo /usr/sbin/nginx -c `pwd`/nginx.conf -s stop', shell=True, cwd='dart-express')
  os.remove('dart-express/nginx.conf')
  #
  # stop dart servers
  #
