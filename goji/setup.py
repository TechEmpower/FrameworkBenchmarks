...
def start(args, logfile, errfile):
  ...
  if os.name == 'nt':
    subprocess.call("set GOPATH=C:\\FrameworkBenchmarks\\goji&& go get ./...", shell=True, cwd="goji", stderr=errfile, stdout=logfile)
    subprocess.Popen("setup.bat", shell=True, cwd="goji", stderr=errfile, stdout=logfile) 
    return 0
  os.environ["GOPATH"] = os.path.expanduser('~/FrameworkBenchmarks/goji')
